...
class Virtual_Machine: public QObject
{
	...
	public:
		...
		enum VmBackend { BackendDirect, BackendLibvirtPassthrough };
		...
		VmBackend Get_Backend() const;
		void Set_Backend( VmBackend backend );
		PassthroughProfile Get_Passthrough_Profile() const;
		void Set_Passthrough_Profile( const PassthroughProfile &profile );
		QString Get_Primary_Disk_Path() const;
		...
		/** Resolve guest USB bus.addr (e.g. "1.2") for host vendor:product id via QEMU monitor. */
		QString Get_USB_Bus_Address( const QString &vendorDeviceId );
		bool Can_Use_Monitor() const;
	...
	private slots:
		...
		QString Query_Monitor_Sync( const QString &cmd, int timeoutMs = 2000 );
	...
	private:
		...
		VmBackend Vm_Backend;
		PassthroughProfile Passthrough_Profile;
