	...
	: QDialog(parent)
{
	...
	CH_GPU_Passthrough = new QCheckBox( tr( "GPU passthrough (Windows / libvirt + IOMMU SR-IOV)" ), this );
	CH_GPU_Passthrough->setToolTip( tr( "Creates a libvirt-backed VM profile for VFIO GPU passthrough and Looking Glass." ) );
	if( ui.Accelerator_Page->layout() )
		ui.Accelerator_Page->layout()->addWidget( CH_GPU_Passthrough );
}
...
bool VM_Wizard_Window::Create_New_VM()
{
	...
	if( CH_GPU_Passthrough && CH_GPU_Passthrough->isChecked() )
	{
		const HostPassthroughReport report = HostHwProbe::scan();
		PassthroughProfile profile;
		profile.gpuBdf = report.recommendedVfBdf;
		if( profile.gpuBdf.isEmpty() && ! report.gpus.isEmpty() )
			profile.gpuBdf = report.gpus.last().bdf;
		profile.useLookingGlass = true;
		profile.kvmfrSizeMb = 32;
		profile.displayMode = PassthroughProfile::DisplayVncInstall;
		profile.machineType = QStringLiteral( "q35" );
		profile.libvirtDomainName = QStringLiteral( "newaqemu-" ) + New_VM->Get_UID();
		New_VM->Set_Backend( Virtual_Machine::BackendLibvirtPassthrough );
		New_VM->Set_Passthrough_Profile( profile );
		New_VM->Set_Machine_Accelerator( VM::KVM );
		New_VM->Set_Video_Card( QStringLiteral( "std" ) );
	}

	// Create New VM XML File
	const QString vmPath = Settings.value( AppInfo::vmDirectoryKey(), "~" ).toString() + VM_File_Name
		+ QStringLiteral( ".aqemu" );
	New_VM->Create_VM_File( vmPath, false );
