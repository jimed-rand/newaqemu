												 ...
												 VM::Emulator_Version version, const QString &internalName )
{
	...
	if( rx.exactMatch(vga_list_string) )
	{
		...
		if( rx.capturedTexts().count() > 1 )
		{
			QStringList vga_devices_list = rx.capturedTexts()[1].split( "|", QStringSkipEmptyParts );
	...
	do
	{
		...
		QStringList net_cards_models = all_models.split( ',', QStringSkipEmptyParts );
...
bool System_Info::Emulator_Supports_Audiodev( const QString &qemuPath )
{
	if( qemuPath.isEmpty() )
		return false;

	const QString out = Get_Emulator_Output( qemuPath, QStringList() << "-audiodev" << "help" );
	return out.contains( "driver", Qt::CaseInsensitive ) &&
	       ! out.contains( "unknown option", Qt::CaseInsensitive ) &&
	       ! out.contains( "not supported", Qt::CaseInsensitive );
}

QStringList System_Info::Probe_Audio_Backends( const QString &qemuPath )
{
	const QStringList candidates = { "pipewire", "pa", "alsa", "sdl", "oss", "wav", "none" };
	QStringList found;

	if( qemuPath.isEmpty() )
		return QStringList() << "pa" << "alsa" << "sdl";

	const QString out = Get_Emulator_Output( qemuPath, QStringList() << "-audiodev" << "help" );
	for( const QString &backend : candidates )
	{
		if( out.contains( backend, Qt::CaseInsensitive ) )
			found << backend;
	}
...
bool System_Info::Update_Host_USB()
{
	...
	for( DWORD index = 0; SetupDiEnumDeviceInfo( devInfo, index, &devData ); ++index )
	{
		...
		if( CM_Get_Device_IDW( devData.DevInst, instanceId, MAX_PATH, 0 ) != CR_SUCCESS )
			continue;

		const QString idLine = QString::fromWCharArray( instanceId );
		if( ! idLine.contains( "VID_", Qt::CaseInsensitive ) )
			continue;
		...
		VM_USB usb;
		usb.Set_ID_Line( idLine );
		usb.Set_Product_Name( QString::fromWCharArray( desc ) );
		All_Host_USB.append( usb );
	}

	SetupDiDestroyDeviceInfoList( devInfo );
	return ! All_Host_USB.isEmpty();
}
