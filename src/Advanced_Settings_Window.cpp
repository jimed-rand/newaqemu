...
{
	ui.setupUi( this );
	UiTooltips::Apply_Advanced_Settings( this );

    settings_widget = new Settings_Widget( ui.All_Tabs, QBoxLayout::TopToBottom, true, false );

	auto *passthroughTab = new Passthrough_Setup_Widget( ui.All_Tabs );
	ui.All_Tabs->addTab( passthroughTab, tr( "GPU passthrough" ) );
...
//	ui.CH_Show_Tab_Info->setChecked( Settings.value("Info/Show_Tab_Info", "yes").toString() == "yes" );
	...
	ui.CH_Audio_Default->setEnabled( true );
	ui.Label_Host_Sound_System->setEnabled( ui.CH_Audio_Default->isChecked() );
	ui.CB_Host_Sound_System->setEnabled( ui.CH_Audio_Default->isChecked() );

	const QString qemuProbe = Settings.value( "QEMU_Path", "qemu-system-x86_64" ).toString();
	const QStringList backends = System_Info::Probe_Audio_Backends( qemuProbe );
	ui.CB_Host_Sound_System->clear();
	ui.CB_Host_Sound_System->addItems( backends );
	...
	connect( ui.CH_Audio_Default, &QCheckBox::toggled, ui.Label_Host_Sound_System, &QWidget::setEnabled );
	connect( ui.CH_Audio_Default, &QCheckBox::toggled, ui.CB_Host_Sound_System, &QWidget::setEnabled );
	...
	ui.CH_Close_To_Tray->setChecked( Settings.value( "Close_To_Tray", "no" ).toString() == "yes" );
	ui.CH_Remember_Emulator_Geometry->setChecked(
		Settings.value( "Remember_Emulator_Control_Geometry", "yes" ).toString() == "yes" );
...
void Advanced_Settings_Window::done(int r)
{
    ...
    //	Settings.setValue( "Info/Show_Tab_Info", ui.CH_Show_Tab_Info->isChecked() ? "yes" : "no" );
	    ...
	    Settings.setValue( "Minimize_To_Tray", ui.CH_Minimize_To_Tray->isChecked() ? "yes" : "no" );
	    Settings.setValue( "Close_To_Tray", ui.CH_Close_To_Tray->isChecked() ? "yes" : "no" );
	    Settings.setValue( "Remember_Emulator_Control_Geometry",
	                        ui.CH_Remember_Emulator_Geometry->isChecked() ? "yes" : "no" );
