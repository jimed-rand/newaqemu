	...
	: QMainWindow( parent )
{
	...
	Apply_Device_Changer_Menu();
	UiTooltips::Apply_Emulator_Control( this );
}

void Emulator_Control_Window::Apply_Device_Changer_Menu()
{
	const bool useNew = Settings.value( "Use_New_Device_Changer", "no" ).toString() == "yes";
	ui.menubar->removeAction( ui.menuConnect->menuAction() );
	ui.menubar->removeAction( ui.menuConnectNew->menuAction() );
	if( useNew )
		ui.menubar->addAction( ui.menuConnectNew->menuAction() );
	else
		ui.menubar->addAction( ui.menuConnect->menuAction() );
}

bool Emulator_Control_Window::Usb_Menus_Enabled() const
{
	return Cur_VM && Cur_VM->Can_Use_Monitor();
}
...
void Emulator_Control_Window::Apply_Full_Size( int w, int h )
{
	if( ! ui.actionDisplay_Scaling->isChecked() )
	{
		...
		{
			if( screen->availableGeometry().width() > w )
...
void Emulator_Control_Window::Eject_Device()
{
	...
	if( act )
	{
		const QString devName = act->data().toString();
		emit Ready_Read_Command( QString( "eject %1" ).arg( devName ) );

		if( devName == "fda" || devName == "floppy0" )
			Cur_VM->Set_FD0( VM_Storage_Device( false, QString() ) );
		else if( devName == "fdb" || devName == "floppy1" )
			Cur_VM->Set_FD1( VM_Storage_Device( false, QString() ) );
		else if( devName == "cdrom" || devName.contains( "-cd" ) )
			Cur_VM->Set_CD_ROM( VM_Storage_Device( false, QString() ) );
	}
}
...
void Emulator_Control_Window::closeEvent( QCloseEvent *event )
{
	...
	Mon_Win->hide();
	Save_Emulator_Geometry();
	event->accept();
}

void Emulator_Control_Window::changeEvent( QEvent *event )
{
...
void Emulator_Control_Window::Save_Emulator_Geometry()
{
	if( Settings.value( "Remember_Emulator_Control_Geometry", "yes" ).toString() != "yes" )
		return;
	Settings.setValue( "Emulator_Control_Geometry", saveGeometry() );
}

void Emulator_Control_Window::Restore_Emulator_Geometry()
{
	if( Settings.value( "Remember_Emulator_Control_Geometry", "yes" ).toString() != "yes" )
		return;
	const QByteArray geom = Settings.value( "Emulator_Control_Geometry" ).toByteArray();
	if( ! geom.isEmpty() )
...
void Emulator_Control_Window::Set_Current_VM( Virtual_Machine *vm )
{
	...
	const bool usbOk = Usb_Menus_Enabled();
	ui.menuUSB_Connect->setEnabled( usbOk );
	ui.menuDisconnect->setEnabled( usbOk );
...
void Emulator_Control_Window::Init()
{
	...
	// Embedded display (VNC or SPICE)
	#ifdef VNC_DISPLAY
	const bool spiceMode = Use_SPICE_Display();
	const bool vncMode = Use_VNC();

	ui.actionDisplay_Scaling->setEnabled( vncMode );
	ui.actionFullscreen_Mode->setEnabled( vncMode );
	ui.actionReinit_VNC->setEnabled( vncMode );
	ui.actionGrab_Mouse->setEnabled( vncMode );

	if( ! vncMode && ! spiceMode )
	{
		/* no embedded display */
	}
	else if( spiceMode )
	{
		...
		if( Machine_View )
		{
			Machine_View->disconnectVNC();
			delete Machine_View;
			Machine_View = nullptr;
		}
		if( ! Spice_View )
			Spice_View = new SpiceView( this );
		setCentralWidget( Spice_View );
		Spice_View->connectToHost( "127.0.0.1", Cur_VM->Get_SPICE().Get_Port() );
		Spice_View->show();
		#endif
	}
	else
	{
		...
		if ( Cur_VM->Use_VNC() )
			Machine_View->Set_VNC_URL( "localhost", Cur_VM->Get_VNC_Display_Number() + 5900 );
		else
	...
	if( usb_item )
	{
		...
		for( int ix = 0; ix < ac_list.count(); ++ix )
		{
			if( usb_item->data().toString() == ac_list[ix]->data().toString() )
			{
				...
				const QString bus_addr =
					Cur_VM->Get_USB_Bus_Address( usb_item->data().toString() );
				
				if( bus_addr.isEmpty() )
				{
					AQGraphic_Error( "void Emulator_Control_Window::Delete_USB_From_VM()",
									 tr( "Error!" ),
									 tr( "Cannot resolve USB bus address. Use Disconnect by Bus Address if needed." ),
									 false );
					return;
				}
...
void Emulator_Control_Window::on_actionUSB_Disconnect_All_Devices_triggered()
{
	...
	for( int dx = 0; dx < ac_list.count(); ++dx )
	{
		...
		{
			const QString bus_addr =
				Cur_VM->Get_USB_Bus_Address( ac_list[dx]->data().toString() );
...
void Emulator_Control_Window::on_actionGrab_Mouse_triggered()
{
	...
	{
		Machine_View->releaseMouseGrab();
		Mouse_Grab_Active = false;
		ui.actionGrab_Mouse->setChecked( false );
	}
	else
	{
		Machine_View->captureAllMouseEvents();
		Mouse_Grab_Active = true;
		ui.actionGrab_Mouse->setCheckable( true );
		ui.actionGrab_Mouse->setChecked( true );
	}
	#else
	Q_UNUSED( this );
	#endif
...
void Emulator_Control_Window::Connect_Display()
{
	QThread::msleep( 500 );
	#ifdef SPICE_DISPLAY
	if( Use_SPICE_Display() && Spice_View )
		Spice_View->launchViewer();
	else
	#endif
	if( Machine_View && Use_VNC() )
		Machine_View->initView();
...
bool Emulator_Control_Window::Use_SPICE_Display() const
{
	return Cur_VM && Cur_VM->Get_SPICE().Use_SPICE();
}
