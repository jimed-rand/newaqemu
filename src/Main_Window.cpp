	...
	: QMainWindow( parent )
{
	...
	Setup_System_Tray();
	UiTooltips::Apply_Main_Window( this );
...
bool Main_Window::Should_Minimize_To_Tray() const
{
	return Tray_Icon && Tray_Icon->isVisible() &&
	       Settings.value( "Minimize_To_Tray", "no" ).toString() == "yes";
}

bool Main_Window::Should_Close_To_Tray() const
{
	return Tray_Icon && Tray_Icon->isVisible() &&
	       Settings.value( "Close_To_Tray", "no" ).toString() == "yes";
}

void Main_Window::Setup_System_Tray()
{
	if( ! QSystemTrayIcon::isSystemTrayAvailable() )
		return;

	Tray_Menu = new QMenu( this );
	Tray_Menu->addAction( tr( "Show newaqemu" ), this, &QWidget::showNormal );
	Tray_Menu->addAction( tr( "Quit" ), qApp, &QApplication::quit );

	Tray_Icon = new QSystemTrayIcon( QIcon( ":/aqemu.png" ), this );
	Tray_Icon->setToolTip( tr( "newaqemu" ) );
...
{
	if( event->type() == QEvent::WindowStateChange && isMinimized() && Should_Minimize_To_Tray() )
	{
		hide();
		if( Tray_Icon )
			Tray_Icon->showMessage( tr( "newaqemu" ), tr( "Minimized to system tray." ),
...
void Main_Window::closeEvent( QCloseEvent *event )
{
	if( Should_Close_To_Tray() )
	{
		...
		event->ignore();
		if( Tray_Icon )
			Tray_Icon->showMessage( tr( "newaqemu" ), tr( "Still running in the system tray." ),
			                        QSystemTrayIcon::Information, 2000 );
		return;
	}
