...
Virtual_Machine::Virtual_Machine( const Virtual_Machine &vm )
{
	...
	this->Vm_Backend = vm.Get_Backend();
	this->Passthrough_Profile = vm.Get_Passthrough_Profile();
...
bool Virtual_Machine::Create_VM_File( const QString &file_name, bool template_mode )
{
	...
	if( template_mode )
		Root_Element = New_Dom_Document.createElement( "newaqemu_template" );
	else
		Root_Element = New_Dom_Document.createElement( "newaqemu" );
	
	Root_Element.setAttribute( "version", AppInfo::version() );
	...
	// Backend / passthrough
	Dom_Element = New_Dom_Document.createElement( "Backend" );
	VM_Element.appendChild( Dom_Element );
	Dom_Text = New_Dom_Document.createTextNode(
		Vm_Backend == BackendLibvirtPassthrough ? "libvirt_passthrough" : "direct" );
	Dom_Element.appendChild( Dom_Text );

	Dom_Element = New_Dom_Document.createElement( "PassthroughProfile" );
	VM_Element.appendChild( Dom_Element );
	QDomElement ptElement = Dom_Element;

	auto appendText = [&]( const QString &tag, const QString &value ) {
		QDomElement e = New_Dom_Document.createElement( tag );
		ptElement.appendChild( e );
		e.appendChild( New_Dom_Document.createTextNode( value ) );
	};
	appendText( "GpuBdf", Passthrough_Profile.gpuBdf );
	appendText( "UseLookingGlass", Passthrough_Profile.useLookingGlass ? "true" : "false" );
	appendText( "KvmfrSizeMb", QString::number( Passthrough_Profile.kvmfrSizeMb ) );
	appendText( "DisplayMode", Passthrough_Profile.displayModeString() );
	appendText( "MachineType", Passthrough_Profile.machineType );
	appendText( "Firmware", Passthrough_Profile.firmware );
	appendText( "LibvirtDomainName", Passthrough_Profile.libvirtDomainName );
	appendText( "UseScreamAudio", Passthrough_Profile.useScreamAudio ? "true" : "false" );
	appendText( "ScreamHostIp", Passthrough_Profile.screamHostIp );
...
bool Virtual_Machine::Load_VM( const QString &file_name )
{
	...
	else
	{
		...
		else
		{
			...
			const QString rootTag = Root_Element.tagName();
			if( rootTag != "AQEMU" && rootTag != "newaqemu" )
			{
				if( rootTag != "AQEMU_Template" && rootTag != "newaqemu_template" )
				{
					AQError( "bool Virtual_Machine::Load_VM( const QString &file_name )",
			...
			for( int ux = 0; ux < usb_count; ++ux )
			{
				...
				{
					QStringList busAddrList = Second_Element.firstChildElement("BusAddr").text().split( ':', QStringSkipEmptyParts );
			...
			const QString backendText = Child_Element.firstChildElement( "Backend" ).text();
			if( backendText == QStringLiteral( "libvirt_passthrough" ) )
				Vm_Backend = BackendLibvirtPassthrough;
			else
				Vm_Backend = BackendDirect;

			{
				const QDomElement pt = Child_Element.firstChildElement( "PassthroughProfile" );
				if( ! pt.isNull() )
				{
					Passthrough_Profile.gpuBdf = pt.firstChildElement( "GpuBdf" ).text();
					Passthrough_Profile.useLookingGlass =
						pt.firstChildElement( "UseLookingGlass" ).text() == "true";
					Passthrough_Profile.kvmfrSizeMb =
						pt.firstChildElement( "KvmfrSizeMb" ).text().toInt();
					Passthrough_Profile.setDisplayModeString(
						pt.firstChildElement( "DisplayMode" ).text() );
					Passthrough_Profile.machineType =
						pt.firstChildElement( "MachineType" ).text();
					Passthrough_Profile.firmware = pt.firstChildElement( "Firmware" ).text();
					Passthrough_Profile.libvirtDomainName =
						pt.firstChildElement( "LibvirtDomainName" ).text();
					Passthrough_Profile.useScreamAudio =
						pt.firstChildElement( "UseScreamAudio" ).text() == "true";
					Passthrough_Profile.screamHostIp =
						pt.firstChildElement( "ScreamHostIp" ).text();
				}
			}
...
QStringList Virtual_Machine::Build_QEMU_Args()
{
	...
	QString qemuBinary;
	{
		const QMap<QString, QString> bin_list = Current_Emulator.Get_Binary_Files();
		if( bin_list.contains( Computer_Type ) )
			qemuBinary = bin_list.value( Computer_Type );
		else if( ! bin_list.isEmpty() )
			qemuBinary = bin_list.constBegin().value();
	}

	const bool useAudiodev = ! qemuBinary.isEmpty() &&
	                         System_Info::Emulator_Supports_Audiodev( qemuBinary );

	if( useAudiodev && audio_list.count() > 0 )
	{
		QString hostDrv = "pa";
		if( Settings.value( "QEMU_AUDIO/Use_Default_Driver", "yes" ).toString() == "no" )
			hostDrv = Settings.value( "QEMU_AUDIO/QEMU_AUDIO_DRV", "pa" ).toString();

		Args << "-audiodev" << hostDrv + ",id=host0";

		if( Audio_Card.Audio_HDA && Current_Emulator_Devices.Audio_Card_List.Audio_HDA )
		{
			Args << "-device" << "intel-hda,audiodev=host0";
			Args << "-device" << "hda-duplex,audiodev=host0";
		}
		else if( Audio_Card.Audio_AC97 && Current_Emulator_Devices.Audio_Card_List.Audio_AC97 )
		{
			Args << "-device" << "AC97,audiodev=host0";
		}
		else if( audio_list.count() > 0 )
		{
			Args << "-soundhw" << audio_list.join( "," );
		}
	}
	else if( audio_list.count() > 0 )
	{
	...
	if( ! Additional_Args.isEmpty() )
	{
		...
		tmp_str.replace( "\n", " " );
		QStringList ad_args = tmp_str.split( " ", QStringSkipEmptyParts );
...
bool Virtual_Machine::Start()
{
	...
	if( Vm_Backend == BackendLibvirtPassthrough )
	{
		if( Passthrough_Profile.gpuBdf.isEmpty() )
		{
			AQGraphic_Error( "bool Virtual_Machine::Start()", tr( "Error!" ),
							 tr( "Passthrough GPU PCI address is not configured." ), false );
			return false;
		}

		if( ! LibvirtRunner::start( *this ) )
		{
			AQGraphic_Error( "bool Virtual_Machine::Start()", tr( "Error!" ),
							 LibvirtRunner::lastError(), false );
			return false;
		}
		...
		if( Passthrough_Profile.useLookingGlass )
		{
...
{
	if( Vm_Backend == BackendLibvirtPassthrough && State == VM::VMS_Running )
	{
		LibvirtRunner::stop( *this );
...
{
	QMap<QString, QString> bin_list = Current_Emulator.Get_Binary_Files();
	QStringList nl = names.split( " ", QStringSkipEmptyParts );
...
bool Virtual_Machine::Can_Use_Monitor() const
{
	if( Get_Backend() != BackendDirect )
		return false;
	if( Get_State() != VM::VMS_Running && Get_State() != VM::VMS_Pause )
		return false;
	return true;
}

QString Virtual_Machine::Query_Monitor_Sync( const QString &cmd, int timeoutMs )
{
	QString command = cmd;
	if( ! command.endsWith( '\n' ) )
		command += '\n';

	Send_Emulator_Command( command );
	...
	while( timer.elapsed() < timeoutMs )
	{
		...
		if( ! Use_Monitor_TCP )
		{
			if( QEMU_Process && QEMU_Process->state() == QProcess::Running )
				output += QString::fromUtf8( QEMU_Process->readAllStandardOutput() );
		}
		else
#endif
		{
			if( Monitor_Socket && Monitor_Socket->bytesAvailable() > 0 )
				output += Monitor_Socket->readAll();
		}

		if( output.contains( "(qemu)" ) || output.contains( "Device " ) ||
			output.contains( "USB device" ) )
			break;
...
QString Virtual_Machine::Get_USB_Bus_Address( const QString &id )
{
	if( ! Can_Use_Monitor() )
	{
		AQError( "QString Virtual_Machine::Get_USB_Bus_Address()",
				 "Monitor not available for this VM backend/state" );
		return QString();
	}

	const QString info_usbhost_res = Query_Monitor_Sync( "info usbhost" );
	if( info_usbhost_res.isEmpty() )
	{
		AQError( "QString Virtual_Machine::Get_USB_Bus_Address()",
				 "info usbhost response empty" );
		return QString();
	}

	const QStringList usbhost_dev_list =
		info_usbhost_res.split( '\n', Qt::SkipEmptyParts );

	RegExpCompat usbhost_dev_reg(
		".+([0-9a-fA-F]{4}:[0-9a-fA-F]{4}),\\s+(.+)" );

	QString dev_name;
	for( const QString &line : usbhost_dev_list )
	{
		if( ! usbhost_dev_reg.exactMatch( line ) )
			continue;

		const QStringList info_lines = usbhost_dev_reg.capturedTexts();
		if( info_lines.size() < 3 )
			continue;

		if( info_lines[1].compare( id, Qt::CaseInsensitive ) == 0 )
		{
			dev_name = info_lines[2].trimmed();
			break;
		}
	}
	...
	{
		AQError( "QString Virtual_Machine::Get_USB_Bus_Address()",
				 "USB device not found in usbhost list: " + id );
		return QString();
	}

	const QString info_usb_res = Query_Monitor_Sync( "info usb" );
	if( info_usb_res.isEmpty() )
	{
		AQError( "QString Virtual_Machine::Get_USB_Bus_Address()",
				 "info usb response empty" );
		return QString();
	}

	const QStringList usb_dev_list = info_usb_res.split( '\n', Qt::SkipEmptyParts );
	RegExpCompat usb_dev_reg(
		"\\s*Device\\s+([0-9]+\\.[0-9]+),\\s+Speed\\s+[0-9]+\\.[0-9]+\\sMb/s,\\sProduct\\s+(.+)" );

	for( const QString &line : usb_dev_list )
	{
		if( ! usb_dev_reg.exactMatch( line ) )
			continue;

		const QStringList info_lines = usb_dev_reg.capturedTexts();
		if( info_lines.size() < 3 )
			continue;

		if( info_lines[2].trimmed() == dev_name )
			return info_lines[1];
	}

	AQError( "QString Virtual_Machine::Get_USB_Bus_Address()",
			 "Bus address not found for: " + dev_name );
	return QString();
}
...
Virtual_Machine::VmBackend Virtual_Machine::Get_Backend() const
{
	return Vm_Backend;
}

void Virtual_Machine::Set_Backend( VmBackend backend )
{
	Vm_Backend = backend;
}

PassthroughProfile Virtual_Machine::Get_Passthrough_Profile() const
{
	return Passthrough_Profile;
}

void Virtual_Machine::Set_Passthrough_Profile( const PassthroughProfile &profile )
{
	Passthrough_Profile = profile;
	if( Passthrough_Profile.libvirtDomainName.isEmpty() )
		Passthrough_Profile.libvirtDomainName = QStringLiteral( "newaqemu-" ) + UID;
}
